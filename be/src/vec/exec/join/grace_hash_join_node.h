...
public:
    ...
    Status add_build_rows(Block* block, const std::vector<uint32_t>& rows, bool eos);

    Status add_probe_rows(RuntimeState* state, Block* block, std::vector<uint32_t>& rows, bool eos);
