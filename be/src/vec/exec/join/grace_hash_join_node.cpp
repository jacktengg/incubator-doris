...
void JoinPartition::close_build(RuntimeState* state) {
    if (in_mem_hash_join_node_) {
        (void)in_mem_hash_join_node_->close(state);
    }
    // build_stream_->close();
}
...
Status JoinPartition::add_build_rows(Block* block, const std::vector<uint32_t>& rows, bool eos) {
...
Status JoinPartition::add_probe_rows(RuntimeState* state, Block* input_block,
                                     std::vector<uint32_t>& rows, bool eos) {
    ...
    while (!eos) {
        ...
        if (status.is<ErrorCode::MEM_LIMIT_EXCEEDED>()) {
            if (in_mem_hash_join_node_->is_merge_build_block_oom()) {
                RETURN_IF_ERROR(build_stream_->add_blocks({std::move(block)}, false));
            }
            auto build_block_ptr = in_mem_hash_join_node_->release_build_block();
            RETURN_IF_ERROR(in_mem_hash_join_node_->close(state));
            in_mem_hash_join_node_.reset();
            auto build_block = *build_block_ptr;
            RETURN_IF_ERROR(build_stream_->add_block(std::move(build_block), false));
            return status;
        }
...
GraceHashJoinNode::GraceHashJoinNode(ObjectPool* pool, const TPlanNode& tnode,
    ...
    io_thread_pool_ = ExecEnv::GetInstance()->spill_stream_mgr()->get_async_task_thread_pool();
...
void GraceHashJoinNode::release_resource(RuntimeState* state) {
    (void)_close_current_partitions(state);
...
Status GraceHashJoinNode::sink(doris::RuntimeState* state, vectorized::Block* input_block,
    ...
    if (eos) {
        VLOG_ROW << "GraceHashJoinNode::sink, eos";
        for (int i = 0; i < PARTITION_COUNT; ++i) {
...
Status GraceHashJoinNode::_partition_build_block(RuntimeState* state, Block* block, bool eos,
    ...
    std::vector<uint32_t> build_partition_rows[PARTITION_COUNT];
...
Status GraceHashJoinNode::push(RuntimeState* state, vectorized::Block* input_block, bool eos) {
    ...
    if (eos) {
        VLOG_ROW << "GraceHashJoinNode::push, eos";
        for (int i = 0; i < PARTITION_COUNT; ++i) {
...
Status GraceHashJoinNode::pull(RuntimeState* state, Block* output_block, bool* eos) {
    ...
    if (is_spilled_probe_) {
        ...
        if (is_building_hash_table_) {
            VLOG_ROW << "GraceHashJoinNode::pull, spilled probe, hash table not ready";
            return Status::OK();
        }
        // need further repartition
        if (_ready_build_partitions_count() == 0) {
            VLOG_ROW << "GraceHashJoinNode::pull, spilled probe, need further repartition";
            return _prepare_spilled_probe(state, eos);
        } else {
            return _probe_spilled_partition(state, output_block, eos);
        }
    } else {
        // VLOG_ROW << "GraceHashJoinNode::pull, initial probe";
        ...
        // probe data finished
        VLOG_ROW << "GraceHashJoinNode::pull, initial probe, push eos";
        for (auto& partition : current_partitions_) {
        ...
        if (spilled_partitions_.empty()) {
            VLOG_ROW << "GraceHashJoinNode::pull, initial probe, all probe eos, sink eos: "
                     << sink_eos_ << ", push eos: " << probe_push_eos_;
            *eos = true;
            return Status::OK();
        }

        VLOG_ROW << "GraceHashJoinNode::pull, initial probe end, start processing spilled "
                    "partitions, build hash tables";
        return _prepare_spilled_probe(state, eos);
    }
}

void GraceHashJoinNode::_calc_columns_hash(
        vectorized::Block* input_block, const std::vector<int>& column_ids,
        std::vector<uint32_t> (&partition2rows)[PARTITION_COUNT]) {
    int rows = input_block->rows();
...
Status GraceHashJoinNode::_partition_probe_block(RuntimeState* state, Block* block, bool eos,
    ...
    std::vector<uint32_t> probe_partition_rows[PARTITION_COUNT];
...
Status GraceHashJoinNode::_build_in_memory_hash_tables_sync(int& success_count) {
    ...
    for (auto& partition : current_partitions_) {
        ...
        if (st.is<ErrorCode::MEM_LIMIT_EXCEEDED>()) {
            RETURN_IF_ERROR(partition->unpin_build_stream());
            break;
        }
        _update_status(st);
        RETURN_IF_ERROR(st);
...
bool GraceHashJoinNode::need_more_probe_data() const {
    ...
    for (auto& partition : current_partitions_) {
        if (partition->is_ready_for_probe() && !partition->need_more_probe_data()) {
            // VLOG_ROW << "GraceHashJoinNode::need_more_input_data, false2";
            return false;
        }
    }

    // VLOG_ROW << "GraceHashJoinNode::need_more_input_data, true";
    return true;
}
bool GraceHashJoinNode::need_more_input_data() const {
    ...
    if (0 == _ready_build_partitions_count()) {
        ...
        auto st = _as_mutable()->_build_hash_tables_async(state_);
        if (!st.ok()) {
            LOG(FATAL) << "_build_hash_tables_async failed: " << st.to_string();
        }
        if (first_push_) {
            _as_mutable()->first_push_ = false;
        }
        VLOG_ROW << "GraceHashJoinNode::need_more_input_data, false1";
        return false;
    }
...
Status GraceHashJoinNode::_repartitioned_probe(RuntimeState* state, Block* output_block,
    ...
    for (auto& partition : current_partitions_) {
        if (partition->is_ready_for_probe() && !partition->current_probe_finished()) {
            bool partition_eos = false;
            VLOG_ROW << "GraceHashJoinNode::pull, probe";
...
Status GraceHashJoinNode::_probe_spilled_partition(RuntimeState* state, Block* output_block,
    ...
    if (!current_partitions_.empty()) {
        // repartitioned
        VLOG_ROW << "GraceHashJoinNode::pull, spilled probe, repartitioned,";
        return _repartitioned_probe(state, output_block, eos);
    } else {
        // not repartitioned
        VLOG_ROW << "GraceHashJoinNode::pull, spilled probe, not repartitioned,";
        bool partition_eos = false;
        ...
        if (partition_eos) {
            VLOG_ROW << "GraceHashJoinNode::pull, spilled probe, not repartitioned, finished "
                        "current partition";
            return _prepare_spilled_probe(state, eos);
        }
    }
...
size_t GraceHashJoinNode::revocable_mem_size(RuntimeState* state) const {
    return 0;
}

Status GraceHashJoinNode::revoke_memory(RuntimeState* state) {
    return Status::OK();
}
} // namespace vectorized
} // namespace doris
