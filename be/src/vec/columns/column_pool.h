...
class ColumnPool {
    ...
    public:
        ...
        void return_object(T* col, size_t block_size) {
            ...
            if (_curr_free.nfree < kBlockSize) {
                ASAN_POISON_MEMORY_REGION(col, sizeof(T));
                _curr_free.ptrs[_curr_free.nfree++] = col;
                _curr_free.bytes += bytes;
...
public:
    ...
    void return_column(T* col, size_t block_size) {
        ...
        if (LIKELY(lp != nullptr)) {
            col->clear();
            lp->return_object(col, block_size);
        } else {
            delete col;
        }
    }
