...
public:
    ...
    void get_all_partitions(std::vector<const SpillPartition*>& partitions) {
        writer_->get_spill_partitions(partitions);
    }

    // prepare to read back spilled data
    Status prepare_for_restore();

    Status clone_partitions_info(std::vector<const SpillPartition*>& partitions);

    // get back spilled blocks, sorted if necessary
...
struct SpillPartition {
    SpillPartition(int32_t id) : id_(id) {}
    int32_t id_;
    bool spilled_ = false;
