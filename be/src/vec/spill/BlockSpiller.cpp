...
Status BlockSpiller::clone_partitions_info(std::vector<const SpillPartition*>& partitions) {
    return assert_cast<PartitionedBlockSpillWriter*>(writer_.get())->clone_partitions_info(partitions);
}
...
Status RawBlockSpillWriter::flush(RuntimeState* state, ThreadPool* thread_pool) {
    ...
    // spill file max size: config::spill_max_file_bytes
...
Status RawBlockSpillWriter::prepare_for_restore() {
    ...
    // k-way merge, until there remains only two big sorted blocks
    // reuse and improve merge algorithm in MergeSorterState
    return Status::OK();
}
...
Status PartitionedBlockSpillWriter::_init_partitions() {
    for (int i = 0; i < partition_count_; ++i) {
        spill_partitions_.emplace_back(std::make_unique<SpillPartition>(i));
        spill_partitions_.back().get()->spill_writer_ = std::make_unique<RawBlockSpillWriter>();
    }
    return Status::OK();
}

Status PartitionedBlockSpillWriter::clone_partitions_info(std::vector<const SpillPartition*>& partitions) {
    spill_partitions_.clear();
    for (auto partition : partitions) {
        spill_partitions_.emplace_back(std::make_unique<SpillPartition>(partition->id_));
        spill_partitions_.back().get()->spill_writer_ = std::make_unique<RawBlockSpillWriter>();
    }
    return Status::OK();
}
