...
TExprNode _make_datetimev2_literal(std::string value, int scale) {
    TExprNode node;
    ...
    TTypeDesc type_desc = create_type_desc(PrimitiveType::TYPE_DATETIMEV2);
    type_desc.types.front().scalar_type.__set_scale(scale);
...
void _build_datetimev2_nano_schema(OperatorContext& ctx, TOlapTableSchemaParam& tschema,
                                   int64_t& schema_index_id) {
    TTypeDesc type_desc = create_type_desc(PrimitiveType::TYPE_DATETIMEV2);
    type_desc.types.front().scalar_type.__set_scale(9);
...
TOlapTablePartition _make_datetimev2_nano_partition(int64_t id, int64_t schema_index_id) {
...
TEST(TabletSinkHashPartitionerTest, DateTimeV2NanoRangeUsesPartitionColumnScale) {
    OperatorContext ctx;

    TOlapTableSchemaParam tschema;
    int64_t schema_index_id = 0;
    _build_datetimev2_nano_schema(ctx, tschema, schema_index_id);
    ...
    auto before_epoch = _make_datetimev2_nano_partition(1, schema_index_id);
    before_epoch.__set_end_keys({_make_datetimev2_literal("1970-01-01 00:00:00", 0)});

    auto epoch = _make_datetimev2_nano_partition(2, schema_index_id);
    epoch.__set_start_keys({_make_datetimev2_literal("1970-01-01 00:00:00", 0)});
    epoch.__set_end_keys({_make_datetimev2_literal("1970-01-01 00:00:00.000000002", 9)});

    auto after_epoch = _make_datetimev2_nano_partition(3, schema_index_id);
    after_epoch.__set_start_keys({_make_datetimev2_literal("1970-01-01 00:00:00.000000002", 9)});
    ...
    const DateTimeV2NanoValue min_value(std::numeric_limits<int64_t>::min());
