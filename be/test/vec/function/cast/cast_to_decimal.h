...
struct FunctionCastToDecimalTest : public FunctionCastTest {
    ...
    void from_string_test_func(int table_index, int& test_data_index, int ScientificExpShift = 0,
        ...
        std::string regression_case_name = fmt::format(
                "test_cast_to_{}_{}_{}_from_str", to_lower(TypeName<T>::get()), Precision, Scale);
    ...
    void from_int_test_func_(int table_index, int& test_data_index, std::ofstream* ofs_case,
        ...
        if (FLAGS_gen_regression_case) {
            ...
            auto table_name =
                    fmt::format("test_cast_to_{}_{}_{}_from_{}", to_lower(TypeName<ToT>::get()),
                                Precision, Scale, to_lower(TypeName<FromT>::get()));
            (*ofs_case) << fmt::format("    sql \"drop table if exists {};\"\n", table_name);
            (*ofs_case) << fmt::format(
    ...
    void from_int_test_func() {
        ...
        std::string regression_case_name =
                fmt::format("test_cast_to_{}_from_int", to_lower(TypeName<DecimalType>::get()));
        if (FLAGS_gen_regression_case) {
            setup_regression_case_output(regression_case_name, ofs_const_case_uptr,
                                         ofs_const_expected_result_uptr, ofs_case_uptr,
    ...
    void from_int_overflow_test_func_(int table_index, int& test_data_index,
        ...
        if constexpr (NumberTraits::max_ascii_len<FromT>() > Precision - Scale) {
            ...
            std::cout << fmt::format("test cast from {} to {}({}, {}) overflow\n",
                                     TypeName<FromT>::get(), TypeName<ToT>::get(), Precision,
                                     Scale);
            ...
            std::set<FromT> test_input_vals = {decimal_max_integral + 1, -decimal_max_integral - 1,
                                               from_max_val, from_min_val};
            // non strict mode
            {
                DataSet data_set;
            ...
            if (FLAGS_gen_regression_case) {
                ...
                auto table_name = fmt::format("test_cast_to_{}_{}_{}_from_{}_overflow",
                                              to_lower(TypeName<ToT>::get()), Precision, Scale,
    ...
    void from_int_overflow_test_func() {
        ...
        std::unique_ptr<std::ofstream> ofs_const_case_uptr, ofs_const_expected_result_uptr;
        std::unique_ptr<std::ofstream> ofs_case_uptr, ofs_expected_result_uptr;
        std::string regression_case_name = fmt::format("test_cast_to_{}_from_int_overflow",
                                                       to_lower(TypeName<DecimalType>::get()));
        if (FLAGS_gen_regression_case) {
            setup_regression_case_output(regression_case_name, ofs_const_case_uptr,
                                         ofs_const_expected_result_uptr, ofs_case_uptr,
                                         ofs_expected_result_uptr, "to_decimal/from_int");
    ...
    void from_float_double_test_func(int table_index, int& test_data_index, std::ofstream* ofs_case,
        ...
        if (FLAGS_gen_regression_case) {
            ...
            auto table_name =
                    fmt::format("test_cast_to_{}_{}_{}_from_{}", to_lower(TypeName<T>::get()),
                                Precision, Scale, to_lower(TypeName<FromT>::get()));
            (*ofs_case) << fmt::format("    sql \"drop table if exists {};\"\n", table_name);
            (*ofs_case) << fmt::format(
                    "    sql \"create table {}(f1 int, f2 {}) "
    ...
    template <PrimitiveType FromPT, typename T, int Precision, int Scale>
    void from_float_double_overflow_test_func(int table_index, int& test_data_index,
                                              std::ofstream* ofs_case,
                                              std::ofstream* ofs_expected_result,
                                              std::ofstream* ofs_const_case,
                                              std::ofstream* ofs_const_expected_result) {
        using FromT = typename PrimitiveTypeTraits<FromPT>::CppType;
        ...
        auto both_int_and_fraction_part_test_func = [&]() {
            std::string dbg_str0 =
                    fmt::format("test cast {} to {}({}, {}), both int and fraction part",
                                TypeName<FromT>::get(), TypeName<T>::get(), Precision, Scale);
            for (const auto& i : integral_part) {
                DataSet data_set;
                std::string dbg_str = dbg_str0;
        ...
        if (FLAGS_gen_regression_case) {
            int start_data_index = test_data_index;
            auto table_name = fmt::format("test_cast_to_{}_{}_{}_from_{}_overflow",
                                          to_lower(TypeName<T>::get()), Precision, Scale,
                                          to_lower(TypeName<FromT>::get()));
            auto value_count = test_input_vals.size();
            auto groovy_var_name = fmt::format("{}_vals", table_name);
            (*ofs_const_case) << fmt::format("    def {} = [", groovy_var_name);
        ...
        if (FLAGS_gen_regression_case) {
            ...
            auto table_name =
                    fmt::format("test_cast_to_{}_{}_{}_from_{}_{}_{}",
                                to_lower(TypeName<ToT>::get()), ToPrecision, ToScale,
    ...
    void between_decimals_with_to_p_and_s_test_func() {
        ...
        std::string regression_case_name =
                fmt::format("test_cast_to_{}_{}_{}_from_{}", to_lower(TypeName<ToT>::get()),
                            ToPrecision, ToScale, to_lower(TypeName<FromT>::get()));
        if (FLAGS_gen_regression_case) {
            setup_regression_case_output(regression_case_name, ofs_const_case_uptr,
                                         ofs_const_expected_result_uptr, ofs_case_uptr,
        ...
        auto test_func = [&]<int test_from_precision>() {
            ...
            if constexpr (test_from_precision != 1) {
                ...
                between_decimal_with_precision_and_scale_test_func<FromT, test_from_precision,
                        ...
                        table_index++, test_data_index, ofs_case, ofs_expected_result,
                        ofs_const_case, ofs_const_expected_result);
                between_decimal_with_precision_and_scale_test_func<
                        FromT, test_from_precision, test_from_precision, ToT, ToPrecision, ToScale>(
                        table_index++, test_data_index, ofs_case, ofs_expected_result,
                        ofs_const_case, ofs_const_expected_result);
            }
        };
    ...
    void between_decimal_with_precision_and_scale_overflow_test_func(
            int from_precision, int from_scale, int table_index, int& test_data_index,
            std::ofstream* ofs_case, std::ofstream* ofs_expected_result,
        ...
        std::string dbg_str0 =
                fmt::format("===============test cast {}({}, {}) to {}({}, {}) overflow: ",
                            TypeName<FromT>::get(), from_precision, from_scale,
                            TypeName<ToT>::get(), ToPrecision, ToScale);
        std::cout << dbg_str0 << std::endl;
        ...
        bool may_overflow =
                (max_to_int_digit_count < max_from_int_digit_count) ||
                (max_to_int_digit_count == max_from_int_digit_count && ToScale < from_scale);
        if (may_overflow) {
            ...
            for (const auto& i : from_integral_part) {
                FromT from_decimal_num {};
                if (i == to_max_integral && ToScale >= from_scale) {
                    // cases: from 999999.999
                    //              xxxxx.xxx
                    //              xxxxx.xxxxx
                    continue;
            ...
            if (FLAGS_gen_regression_case) {
                ...
                auto table_name =
                        fmt::format("test_cast_to_{}_{}_{}_from_{}_{}_{}",
                                    to_lower(TypeName<ToT>::get()), ToPrecision, ToScale,
                                    to_lower(TypeName<FromT>::get()), from_precision, from_scale);
                auto value_count = test_input_vals.size();
    ...
    void between_decimals_with_to_p_and_s_overflow_test_func(
        ...
        for (const auto& from_p_s : from_precision_scales) {
            between_decimal_with_precision_and_scale_overflow_test_func<FromT, ToT, ToPrecision,
                                                                        ToScale>(
                    from_p_s.first, from_p_s.second, table_index++, test_data_index, ofs_case,
    ...
    void between_decimal_overflow_test_func() {
        ...
        auto test_func = [&]<int test_to_precision>() {
            ...
            std::unique_ptr<std::ofstream> ofs_case_uptr, ofs_expected_result_uptr;
            std::string regression_case_name = fmt::format(
                    "test_cast_to_{}_{}_from_{}_overflow", to_lower(TypeName<ToT>::get()),
                    test_to_precision, to_lower(TypeName<FromT>::get()));
            if (FLAGS_gen_regression_case) {
                setup_regression_case_output(regression_case_name, ofs_const_case_uptr,
                                             ofs_const_expected_result_uptr, ofs_case_uptr,
                                             ofs_expected_result_uptr, "to_decimal/from_decimal");
            ...
            between_decimals_with_to_p_and_s_overflow_test_func<FromT, ToT, test_to_precision, 0>(
                    table_index, test_data_index, ofs_case, ofs_expected_result, ofs_const_case,
                    ofs_const_expected_result);
            between_decimals_with_to_p_and_s_overflow_test_func<FromT, ToT, test_to_precision, 1>(
                    table_index, test_data_index, ofs_case, ofs_expected_result, ofs_const_case,
                    ofs_const_expected_result);
            if constexpr (test_to_precision != 1) {
                between_decimals_with_to_p_and_s_overflow_test_func<FromT, ToT, test_to_precision,
                                                                    test_to_precision / 2>(
                        table_index, test_data_index, ofs_case, ofs_expected_result, ofs_const_case,
                        ofs_const_expected_result);
                between_decimals_with_to_p_and_s_overflow_test_func<FromT, ToT, test_to_precision,
