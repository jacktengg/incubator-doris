...
TEST_F(ColumnZoneMapTest, DateTimeV2NanoPage) {
    auto column = std::make_shared<TabletColumn>();
    column->_unique_id = 0;
    column->_col_name = "datetimev2_nano";
    column->_type = FieldType::OLAP_FIELD_TYPE_DATETIMEV2_NANO;
    column->_is_key = true;
    ...
    auto data_type =
            DataTypeFactory::instance().create_data_type(TYPE_DATETIMEV2_NANO, false, 0, 9);
    std::unique_ptr<ZoneMapIndexWriter> writer;
    ASSERT_TRUE(ZoneMapIndexWriter::create(data_type, column.get(), writer).ok());

    DateTimeV2NanoValue values[] = {
            DateTimeV2NanoValue(std::numeric_limits<int64_t>::min()),
            DateTimeV2NanoValue(-1),
            DateTimeV2NanoValue(0),
            DateTimeV2NanoValue(std::numeric_limits<int64_t>::max()),
    };
    writer->add_values(values, std::size(values));
    ASSERT_TRUE(writer->flush().ok());

    const std::string file_path = kTestDir + "/datetimev2_nano_zonemap";
    io::FileWriterPtr file_writer;
    ASSERT_TRUE(_fs->create_file(file_path, &file_writer).ok());
    ColumnIndexMetaPB index_meta;
    ASSERT_TRUE(writer->finish(file_writer.get(), &index_meta).ok());
    ...
    ASSERT_TRUE(ZoneMap::from_proto(index_meta.zone_map_index().segment_zone_map(), data_type,
                                    segment_zone_map)
                        .ok());
    EXPECT_EQ(segment_zone_map.min_value.get<TYPE_DATETIMEV2_NANO>(), values[0]);
    EXPECT_EQ(segment_zone_map.max_value.get<TYPE_DATETIMEV2_NANO>(), values[3]);
}
